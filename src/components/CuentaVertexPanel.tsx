...
/**
 * "Cuenta de Vertex": cada usuario ve con que cuenta genera, carga la suya y aprende a
 * sacarla. Es una pestaña de ConfiguracionDialog, que se abre desde el nombre, arriba a
 * la derecha (SessionBar).
 *
...
import { Badge, Button, Confirmar, Input } from "@/components/ui";
...
export default function CuentaVertexPanel({
  estado,
  onEstado,
}: {
  estado: EstadoCuentaVertex;
  ...
  // Al montarse (abrir el dialogo o volver a esta pestaña, que Radix desmonta al salir):
  // el estado fresco, que pudo cambiar en otra pestaña del navegador. Lo demas ya
  // arranca limpio porque el componente es nuevo.
  useEffect(() => {
    llamar("GET")
    ...
    // Solo al montarse: `onEstado` cambia justamente por este efecto.
    ...
    <>
      <div className="flex flex-col gap-4">
        <p className="text-body text-fg-dim">
          Con esta cuenta de Google Cloud se generan tus imágenes y videos, y a ella se le factura.
        </p>
        {estado.modo === "mock" && (
